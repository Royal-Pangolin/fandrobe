...
<div class="container-fluid px-4 px-lg-5 py-5">
    <div class="admin-container-sm">
        ...
        <div class="mb-5">
            ...
            <h1 class="fw-bolder mb-0">{{ __('messages.new_artist') }}</h1>
        </div>
        ...
        <form action="{{ route('admin.artistas.store') }}" method="POST">
            ...
            <div class="d-flex flex-column gap-4">
                ...
                <div>
                    <label class="form-label fw-bold small text-uppercase admin-form-label">{{ __('messages.genre') }} *</label>
                ...
                <div>
                    <label class="form-label fw-bold small text-uppercase admin-form-label">{{ __('messages.bio') }}</label>
                    <textarea name="bio" rows="4"
                              class="form-control rounded-3 @error('bio') is-invalid @enderror"
                              >{{ old('bio') }}</textarea>
                ...
                <div class="d-flex align-items-center gap-3 p-3 rounded-3 admin-checkbox-row">
                    ...
                    <label for="is_active" class="form-check-label fw-bold">{{ __('messages.active_artist') }}</label>
                </div>

                <div class="d-flex gap-3 pt-2">
                    <button type="submit" class="btn btn-primary fw-bold px-5 admin-btn-submit">
                        {{ __('messages.create_artist') }}
                    </button>
