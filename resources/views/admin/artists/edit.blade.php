@extends('layouts.app')
@section('title', __('messages.edit_artist') . ' — Admin')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-12">
        ...
        <h1 class="text-4xl font-extrabold text-shadow tracking-tight">{{ __('messages.edit_artist') ?? 'Editar artista' }}</h1>
    </div>
    ...
    <div class="bg-shadow/5 border border-shadow/5 p-8 rounded-3xl">
        <form action="{{ route('admin.artistas.update', $artist->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="bio" rows="4"
                          class="w-full bg-primary border @error('bio') border-error @else border-shadow/20 @enderror text-shadow text-sm rounded-xl focus:ring-2 focus:ring-accent focus:border-accent block p-3 font-bold">{{ old('bio', $artist->bio) }}</textarea>
            ...
            <div class="bg-shadow/5 p-4 rounded-xl flex items-center gap-3 cursor-pointer group" onclick="document.getElementById('is_active').click();">
                ...
                <input type="checkbox" name="is_active" value="1" id="is_active"
                       ...
                       {{ old('is_active', $artist->is_active) ? 'checked' : '' }} onclick="event.stopPropagation();">
