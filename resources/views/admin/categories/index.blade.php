@extends('layouts.app')
@section('title', __('messages.admin_categories') . ' — Admin')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
        <div>
            ...
            <h1 class="text-4xl font-extrabold text-shadow tracking-tight">Categorías</h1>
        </div>
        <a href="{{ route('admin.categorias.create') }}" class="btn-primary py-2 px-6 shadow-sm">
            + Nueva categoría
        </a>
    </div>
            ...
            <table class="w-full text-left whitespace-nowrap">
                <thead class="bg-shadow/5 border-b border-shadow/10">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-muted uppercase tracking-widest">Nombre</th>
                        <th class="px-6 py-4 text-xs font-bold text-muted uppercase tracking-widest">Categoría padre</th>
                        <th class="px-6 py-4 text-xs font-bold text-muted uppercase tracking-widest">Productos</th>
                ...
                <tbody class="divide-y divide-shadow/10">
                    @forelse($categories as $category)
                        <tr class="hover:bg-shadow/5 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right flex items-center justify-end gap-2">
                                <a href="{{ route('admin.categorias.edit', $category->id) }}" class="inline-flex items-center justify-center px-4 py-2 border-2 border-shadow/20 rounded-full text-sm font-bold text-shadow hover:bg-shadow/5 hover:border-shadow/40 transition-all">
                                    Editar
                                </a>
                                <form action="{{ route('admin.categorias.destroy', $category->id) }}" method="POST"
                                      onsubmit="return confirm('¿Eliminar esta categoría?')" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-muted font-medium">
